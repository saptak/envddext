...
const fallbackTemplates: Record<string, TemplateMetadata> = {
  "basic-http-echo": {
    ...
    description:
      "Deploy a simple echo service with HTTP routing through Envoy Gateway",
    category: "basic-http",
    difficulty: "beginner",
  ...
  "tls-termination": {
    ...
    description:
      "Secure your services with HTTPS using TLS termination at the Gateway",
    category: "tls",
  ...
  "traffic-splitting": {
    ...
    description:
      "Route traffic to multiple versions of a service with weighted routing",
...
export const fetchTemplatesMetadata = async (): Promise<TemplateMetadata[]> => {
  try {
    ...
    if (!response.ok) {
      console.warn(
        `Failed to fetch templates from GitHub: ${response.statusText}`,
      );
...
export const loadTemplate = async (id: string): Promise<Template | null> => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Failed to fetch template YAML from GitHub: ${response.statusText}`,
      );
      return null;
    }
...
export const applyTemplateFromUrl = async (
  ...
  try {
    ...
    // Use the host's CLI to apply the template
    console.log(`Applying template via host CLI from URL: ${url}`);
    ...
    console.log("Host CLI apply result:", result);

    // Check if the operation was successful based on kubectl output.
    // `kubectl apply` can output to stdout or stderr for success-like messages (e.g., "unchanged", "configured").
    const stdout = result.stdout || "";
    const stderr = result.stderr || ""; // Ensure stderr is a string, default to empty
    ...
    if (isSuccessOperation) {
      // If any success keyword is found in stdout or stderr, consider it a success.
      console.log(
        `Template application deemed successful. Stdout: ${stdout}, Stderr: ${stderr}`,
      );
      return { success: true };
    } else {
      // Otherwise, if no success keywords are found, it's considered a failure.
      // The presence of stderr content without success keywords is a strong indicator of an error.
      // If stderr is empty but stdout also lacks success keywords, it's also treated as a failure for `kubectl apply`.
      const errorMessage = `Failed to apply template. No success keywords found in output.\\nStdout: ${stdout}\\nStderr: ${stderr}`;
  ...
  try {
    ...
    if (!crdOutput?.stdout || crdOutput.stdout.trim() === "") {
      ...
      return {
        success: false,
        error:
          "Gateway API CRDs are not installed. Please make sure Envoy Gateway is properly installed.",
      };
    }
...
spec:
  controllerName: gateway.envoyproxy.io/gatewayclass-controller`;
      ...
      const gatewayClassUrl =
        "https://raw.githubusercontent.com/saptak/envoygatewaytemplates/main/gatewayclass.yaml";
  ...
  try {
    ...
    if (!gcOutput?.stdout || gcOutput.stdout.trim() === "") {
      ...
      return {
        status: "failed",
        message:
          'GatewayClass "envoy-gateway" not found. This is required for the Gateway to function.',
      };
    }
    ...
    try {
      ...
      console.log(
        `Deployment replicas: ${availableReplicas}/${desiredReplicas}`,
      );
