...
**SAN FRANCISCO, CA - June 2025** - Developers struggling with complex API gateway configurations now have a powerful new solution. The Envoy Gateway Docker Desktop Extension v0.9.0 transforms what was once a time-consuming, error-prone process into a simple, visual workflow that gets developers productive in minutes, now featuring comprehensive security policy management, advanced user experience enhancements, and enterprise-grade authentication capabilities alongside the consolidated Dashboard, synthetic traffic generation, and performance testing tools.
...
Picture this: you open Docker Desktop, click a few buttons, and within minutes you have a production-ready API gateway running locally with comprehensive security policies, sophisticated traffic splitting capabilities, and enterprise-grade authentication. The intuitive eight-tab interface provides everything from basic routing to advanced security management. No YAML files to debug, no complex networking to configure, no hours lost to documentation. This is the reality the Envoy Gateway Extension v0.9.0 delivers.
...
The groundbreaking v0.9.0 release introduces comprehensive security policy management and enhanced user experience features that bring enterprise-grade security to every developer. The new Security Policies tab provides intuitive interfaces for Basic Authentication, CORS policies, IP filtering, and Mutual TLS (mTLS) - turning complex PKI configurations into guided step-by-step wizards. The Enhanced Template Gallery with search and filtering capabilities, combined with the new Resource Creation Wizard and professional YAML Editor, makes complex configurations accessible to developers at every skill level.
...
Current capabilities span the complete gateway lifecycle: creating and managing Gateways, setting up HTTP routing rules with advanced traffic splitting, comprehensive security policy management including Basic Authentication and mTLS, TLS certificate management with automated cert-manager installation, and professional monitoring with visual resource cards all accessible through the intuitive eight-tab interface. The extension excels at getting teams up and running with sophisticated deployment patterns and enterprise security quickly, handling the most common use cases that traditionally required extensive Kubernetes, DevOps, and security expertise.
...
The v0.9.0 security policy features have proven equally revolutionary. "Setting up Basic Authentication used to require deep Kubernetes Secret knowledge and manual YAML configuration," explains Security Engineer Marcus Rodriguez. "Now our frontend developers can protect routes with authentication policies using guided forms. The mTLS wizard walked us through complex PKI setup that would have taken our security team days to configure manually."
...
The development team continues responding to user feedback with an ambitious roadmap. With traffic splitting, TLS management, and comprehensive security policy management now complete, the next major milestone includes resource editing capabilities, allowing users to modify existing Gateways and HTTPRoutes directly in the interface. Following that, advanced policy features will introduce JWT authentication, rate limiting, and advanced traffic policies through the same intuitive interface that makes security and routing patterns so accessible.
