...
**SAN FRANCISCO, CA - June 2025** - Developers struggling with complex API gateway configurations now have a powerful new solution. The Envoy Gateway Docker Desktop Extension v0.9.1 transforms what was once a time-consuming, error-prone process into a simple, visual workflow that gets developers productive in minutes, now featuring comprehensive rate limiting and advanced traffic control, security policy management, enhanced user experience, and enterprise-grade authentication capabilities alongside the consolidated Dashboard, synthetic traffic generation, and performance testing tools.
...
Picture this: you open Docker Desktop, click a few buttons, and within minutes you have a production-ready API gateway running locally with comprehensive rate limiting policies, sophisticated traffic splitting capabilities, advanced security management, and enterprise-grade authentication. The intuitive eight-tab interface provides everything from basic routing to advanced rate limiting and traffic control. No YAML files to debug, no complex networking to configure, no hours lost to documentation. This is the reality the Envoy Gateway Extension v0.9.1 delivers.
...
The revolutionary v0.9.1 release introduces comprehensive rate limiting and advanced traffic control that brings enterprise-grade protection to every developer. The enhanced Security Policies tab now provides intuitive interfaces for Basic Authentication, CORS policies, IP filtering, Mutual TLS (mTLS), and sophisticated rate limiting - turning complex traffic protection into guided step-by-step wizards. Multi-dimensional rate limiting (global, per-IP, per-header, per-user) with burst testing capabilities, combined with the Enhanced Template Gallery, Resource Creation Wizard, and professional YAML Editor, makes complex traffic control accessible to developers at every skill level.
...
Current capabilities span the complete gateway lifecycle: creating and managing Gateways, setting up HTTP routing rules with advanced traffic splitting, comprehensive security policy management including Basic Authentication, mTLS, and sophisticated rate limiting, TLS certificate management with automated cert-manager installation, and professional monitoring with visual resource cards all accessible through the intuitive eight-tab interface. The extension excels at getting teams up and running with sophisticated deployment patterns, enterprise security, and advanced traffic control quickly, handling the most common use cases that traditionally required extensive Kubernetes, DevOps, and security expertise.
...
The development team continues responding to user feedback with an ambitious roadmap. With traffic splitting, TLS management, comprehensive security policy management, and advanced rate limiting now complete, the next major milestone includes resource editing capabilities, allowing users to modify existing Gateways and HTTPRoutes directly in the interface. Following that, advanced policy features will introduce JWT authentication and additional advanced traffic policies through the same intuitive interface that makes security and routing patterns so accessible.
